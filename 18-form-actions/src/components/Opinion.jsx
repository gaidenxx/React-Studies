...
export function Opinion({ opinion: { id, title, body, userName, votes } }) {
  ...
  // Hook used to create temporary values
  const [optimisticVotes, setVotesOptimistically] = useOptimistic(votes, 
    (prevVotes, mode) => mode === 'up' ? 
      prevVotes + 1 : prevVotes - 1
  );
  ...
  const [upvoteFormState, upvoteFormAction, upvotePending] = useActionState(upvoteAction, null); // null or none because there is no formstate in this situation
